...
- Stage 2 — reranking: a reranker (typically a [cross-encoder](https://www.sbert.net/examples/cross_encoder/applications/README.html)) takes the query paired with each candidate chunk and scores relevance directly. It is more accurate than embedding similarity — it reads query and chunk together rather than comparing vectors computed separately — but too slow to run over the whole knowledge base, so it only rescores the stage-1 candidates and passes the best few to the LLM.  

A **cross-encoder** is a model that takes two texts as a single joined input — here, the query and a candidate chunk concatenated together (e.g., [CLS] query [SEP] chunk) — and runs them through a [transformer](transformer.md] jointly, outputting a single relevance score.  
